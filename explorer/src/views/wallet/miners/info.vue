...
import {getMinerTokenID} from "@/api/contract_utils";
...
export default {
  ...
  methods: {
    gotoOpenSea(){
      window.open(this.data.nftInfo.url,'_blank');
    },
    async handleIsNft() {
      this.data.nftInfo = await getMinerTokenID(this.$route.params.address);
    },
    handleGetPageInfo(){
